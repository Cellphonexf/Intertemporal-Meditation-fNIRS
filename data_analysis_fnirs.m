...
%% Programmed by Feng Xiao (2024.6.4)
clear all,
clc,
%% Parameter settings
samplingrate = 5.1;
...
%% Figure 5 Plotting (Bilateral hemodynamic activation contrast between two meditations)
% cd ..\..\..
cd Figure5\
...
%% Bilateral activation comparisons (intertemporal)
left_ch = [16 17 11 12 1 6 7 18 22 29 35];
right_ch = [15 19 8 9 2 4 5 20 24 32 36];
label_ba = [9 9 10 10 11 11 46 45 21 21 17];
L_intertemporal = [];
L_intertemporal_ses1 = [];
L_intertemporal_ses2 = [];
L_intertemporal_ses3 = [];
R_intertemporal = [];
R_intertemporal_ses1 = [];
R_intertemporal_ses2 = [];
R_intertemporal_ses3 = [];

for i = left_ch
    temp_left = Total_intertemporal_hbo(:, i);
    L_intertemporal = [L_intertemporal, temp_left];
    clear temp_left
    
    temp_left = Ses1_intertemporal_hbo(:, i);
    L_intertemporal_ses1 = [L_intertemporal_ses1, temp_left];
    clear temp_left
    
    temp_left = Ses2_intertemporal_hbo(:, i);
    L_intertemporal_ses2 = [L_intertemporal_ses2, temp_left];
    clear temp_left
    
    temp_left = Ses3_intertemporal_hbo(:, i);
    L_intertemporal_ses3 = [L_intertemporal_ses3, temp_left];
    clear temp_left
end

for i = right_ch
    temp_right = Total_intertemporal_hbo(:, i);
    R_intertemporal = [R_intertemporal, temp_right];
    clear temp_right
    
    temp_right = Ses1_intertemporal_hbo(:, i);
    R_intertemporal_ses1 = [R_intertemporal_ses1, temp_right];
    clear temp_right
    
    temp_right = Ses2_intertemporal_hbo(:, i);
    R_intertemporal_ses2 = [R_intertemporal_ses2, temp_right];
    clear temp_right
    
    temp_right = Ses3_intertemporal_hbo(:, i);
    R_intertemporal_ses3 = [R_intertemporal_ses3, temp_right];
    clear temp_right
end

bi_intertemporal = [label_ba; zeros(1,11)]; %col2:p; col3:t-value; col4: efs; 
bi_intertemporal_ses1 = [label_ba; zeros(1,11)];
bi_intertemporal_ses2 = [label_ba; zeros(1,11)];
bi_intertemporal_ses3 = [label_ba; zeros(1,11)];

for i = 1:size(label_ba,2)
    [h,p,ci,stats] = ttest(L_intertemporal(:,i), R_intertemporal(:,i)); %left - right; paired ttest
    temp_mean_df = mean(L_intertemporal(:,i)) - mean(R_intertemporal(:,i));
    std_left = std(L_intertemporal(:,i));
    std_right = std(R_intertemporal(:,i));
    temp_poolstd = sqrt((std_left^2 + std_right^2) / 2);
    bi_intertemporal(2,i) = p;
    bi_intertemporal(3,i) = stats.tstat;
    bi_intertemporal(4,i) = temp_mean_df / temp_poolstd; %left - right; Cohen's d
    clear h p ci stats temp_mean_df std_left std_right temp_poolstd
    
    [h,p,ci,stats] = ttest(L_intertemporal_ses1(:,i), R_intertemporal_ses1(:,i)); %left - right; paired ttest
    temp_mean_df = mean(L_intertemporal_ses1(:,i)) - mean(R_intertemporal_ses1(:,i));
    std_left = std(L_intertemporal_ses1(:,i));
    std_right = std(R_intertemporal_ses1(:,i));
    temp_poolstd = sqrt((std_left^2 + std_right^2) / 2);
    bi_intertemporal_ses1(2,i) = p;
    bi_intertemporal_ses1(3,i) = stats.tstat;
    bi_intertemporal_ses1(4,i) = temp_mean_df / temp_poolstd; %left - right; Cohen's d
    clear h p ci stats temp_mean_df std_left std_right temp_poolstd
    
    [h,p,ci,stats] = ttest(L_intertemporal_ses2(:,i), R_intertemporal_ses2(:,i)); %left - right; paired ttest
    temp_mean_df = mean(L_intertemporal_ses2(:,i)) - mean(R_intertemporal_ses2(:,i));
    std_left = std(L_intertemporal_ses2(:,i));
    std_right = std(R_intertemporal_ses2(:,i));
    temp_poolstd = sqrt((std_left^2 + std_right^2) / 2);
    bi_intertemporal_ses2(2,i) = p;
    bi_intertemporal_ses2(3,i) = stats.tstat;
    bi_intertemporal_ses2(4,i) = temp_mean_df / temp_poolstd; %left - right; Cohen's d
    clear h p ci stats temp_mean_df std_left std_right temp_poolstd
    
    [h,p,ci,stats] = ttest(L_intertemporal_ses3(:,i), R_intertemporal_ses3(:,i)); %left - right; paired ttest
    temp_mean_df = mean(L_intertemporal_ses3(:,i)) - mean(R_intertemporal_ses3(:,i));
    std_left = std(L_intertemporal_ses3(:,i));
    std_right = std(R_intertemporal_ses3(:,i));
    temp_poolstd = sqrt((std_left^2 + std_right^2) / 2);
    bi_intertemporal_ses3(2,i) = p;
    bi_intertemporal_ses3(3,i) = stats.tstat;
    bi_intertemporal_ses3(4,i) = temp_mean_df / temp_poolstd; %left - right; Cohen's d
    clear h p ci stats temp_mean_df std_left std_right temp_poolstd
end
%% Bilateral activation comparisons (mindfulness)
L_mindfulness = [];
L_mindfulness_ses1 = [];
L_mindfulness_ses2 = [];
L_mindfulness_ses3 = [];
...
for i = left_ch
    temp_left = Total_mindfulness_hbo(:, i);
    L_mindfulness = [L_mindfulness, temp_left];
    clear temp_left
    
    temp_left = Ses1_mindfulness_hbo(:, i);
    L_mindfulness_ses1 = [L_mindfulness_ses1, temp_left];
    clear temp_left
    
    temp_left = Ses2_mindfulness_hbo(:, i);
    L_mindfulness_ses2 = [L_mindfulness_ses2, temp_left];
    clear temp_left
    
    temp_left = Ses3_mindfulness_hbo(:, i);
    L_mindfulness_ses3 = [L_mindfulness_ses3, temp_left];
    clear temp_left
end

for i = right_ch
    temp_right = Total_mindfulness_hbo(:, i);
    R_mindfulness = [R_mindfulness, temp_right];
    clear temp_right
    
    temp_right = Ses1_mindfulness_hbo(:, i);
    R_mindfulness_ses1 = [R_mindfulness_ses1, temp_right];
    clear temp_right
    
    temp_right = Ses2_mindfulness_hbo(:, i);
    R_mindfulness_ses2 = [R_mindfulness_ses2, temp_right];
    clear temp_right
    
    temp_right = Ses3_mindfulness_hbo(:, i);
    R_mindfulness_ses3 = [R_mindfulness_ses3, temp_right];
    clear temp_right
end

bi_mindfulness = [label_ba; zeros(1,11)]; %col2:p; col3:t-value; col4: efs; 
bi_mindfulness_ses1 = [label_ba; zeros(1,11)];
bi_mindfulness_ses2 = [label_ba; zeros(1,11)];
bi_mindfulness_ses3 = [label_ba; zeros(1,11)];

for i = 1:size(label_ba,2)
    [h,p,ci,stats] = ttest(L_mindfulness(:,i), R_mindfulness(:,i)); %left - right; paired ttest
    temp_mean_df = mean(L_mindfulness(:,i)) - mean(R_mindfulness(:,i));
    std_left = std(L_mindfulness(:,i));
    std_right = std(R_mindfulness(:,i));
    temp_poolstd = sqrt((std_left^2 + std_right^2) / 2);
    bi_mindfulness(2,i) = p;
    bi_mindfulness(3,i) = stats.tstat;
    bi_mindfulness(4,i) = temp_mean_df / temp_poolstd; %left - right; Cohen's d
    clear h p ci stats temp_mean_df std_left std_right temp_poolstd
    
    [h,p,ci,stats] = ttest(L_mindfulness_ses1(:,i), R_mindfulness_ses1(:,i)); %left - right; paired ttest
    temp_mean_df = mean(L_mindfulness_ses1(:,i)) - mean(R_mindfulness_ses1(:,i));
    std_left = std(L_mindfulness_ses1(:,i));
    std_right = std(R_mindfulness_ses1(:,i));
    temp_poolstd = sqrt((std_left^2 + std_right^2) / 2);
    bi_mindfulness_ses1(2,i) = p;
    bi_mindfulness_ses1(3,i) = stats.tstat;
    bi_mindfulness_ses1(4,i) = temp_mean_df / temp_poolstd; %left - right; Cohen's d
    clear h p ci stats temp_mean_df std_left std_right temp_poolstd
    
    [h,p,ci,stats] = ttest(L_mindfulness_ses2(:,i), R_mindfulness_ses2(:,i)); %left - right; paired ttest
    temp_mean_df = mean(L_mindfulness_ses2(:,i)) - mean(R_mindfulness_ses2(:,i));
    std_left = std(L_mindfulness_ses2(:,i));
    std_right = std(R_mindfulness_ses2(:,i));
    temp_poolstd = sqrt((std_left^2 + std_right^2) / 2);
    bi_mindfulness_ses2(2,i) = p;
    bi_mindfulness_ses2(3,i) = stats.tstat;
    bi_mindfulness_ses2(4,i) = temp_mean_df / temp_poolstd; %left - right; Cohen's d
    clear h p ci stats temp_mean_df std_left std_right temp_poolstd
    
    [h,p,ci,stats] = ttest(L_mindfulness_ses3(:,i), R_mindfulness_ses3(:,i)); %left - right; paired ttest
    temp_mean_df = mean(L_mindfulness_ses3(:,i)) - mean(R_mindfulness_ses3(:,i));
    std_left = std(L_mindfulness_ses3(:,i));
    std_right = std(R_mindfulness_ses3(:,i));
    temp_poolstd = sqrt((std_left^2 + std_right^2) / 2);
    bi_mindfulness_ses3(2,i) = p;
    bi_mindfulness_ses3(3,i) = stats.tstat;
    bi_mindfulness_ses3(4,i) = temp_mean_df / temp_poolstd; %left - right; Cohen's d
    clear h p ci stats temp_mean_df std_left std_right temp_poolstd
end
%% Figure A.1 Plotting (HbO amplitude fluctuation in line graph)
cd ..\..\..
cd FigureA1\
mediPeriodLabels = {'Ses 1','Ses 2','Ses 3'};
for i = [1:9,11:12,14:20,22,24,26,29,32,35:36] %25 valid channels; channel 28 with extreme variation was excluded
    main_plot = figure('color', 'w');
    set(main_plot, 'Units', 'inches', 'Position', [0, 0, 2, 1.5]);
    temp_hbo_m = [hbo_mindfulness_ses1(1,i),hbo_mindfulness_ses2(1,i),hbo_mindfulness_ses3(1,i)];
    temp_hbo_m_se = [hbo_mindfulness_ses1(2,i),hbo_mindfulness_ses2(2,i),hbo_mindfulness_ses3(2,i)];
    temp_hbo_i = [hbo_intertemporal_ses1(1,i),hbo_intertemporal_ses2(1,i),hbo_intertemporal_ses3(1,i)];
    temp_hbo_i_se = [hbo_intertemporal_ses1(2,i),hbo_intertemporal_ses2(2,i),hbo_intertemporal_ses3(2,i)];
    plot(1:3,temp_hbo_m,'Color',[0.3, 0.4, 0.9],'linewidth',0.5); hold on
    errorbar(1:3, temp_hbo_m, temp_hbo_m_se, 'Color',[0.3, 0.4, 0.9], 'linewidth', 0.5);
    plot(1:3,temp_hbo_i,'Color',[0.7, 0.1, 0.1],'linewidth',0.5)
    errorbar(1:3, temp_hbo_i, temp_hbo_i_se, 'Color',[0.7, 0.1, 0.1], 'linewidth', 0.5);
    title(['CH',num2str(i)]);
    xlabel('', 'Color', 'k');
    ylabel('HbO amplitude (uM)', 'Color', 'k');
    ylim([-60 60]);
    ...
    xticklabels(mediPeriodLabels);
    line([0 5], [0 0], 'Color', [0 0 0], 'LineStyle', '--');
    set(gca,'fontsize',7,'FontName','Times New Roman', 'XColor', 'k', 'YColor', 'k');
    set(findall(gcf,'type','line'),'LineWidth',0.5, 'Color', 'k');
    set(get(gca,'xlabel'),'fontsize',7);
    set(get(gca,'ylabel'),'fontsize',7);
    ...
    grid off
    box off
    print(main_plot,['ch',num2str(i)],'-dpdf','-r600')
    close all
end
%% Figure 5 Plotting (HbO changes in time domain)
%% DLPFC
cd ..\..\..
cd Figure5\
durTotal = (1:size(Total_mindfulness_hbo,1))/samplingrate; %convert time frames to seconds
for i = [17,19,7,5]
ch_plot = figure('color', 'w');
set(ch_plot, 'Units', 'inches', 'Position', [0, 0, 2, 1.5]);
fill([durTotal, fliplr(durTotal)], ...
         [Total_mindfulness_hbo_upper(:,i); flipud(Total_mindfulness_hbo_lower(:,i))], ...
         [0.3, 0.4, 0.9], 'LineStyle', 'none', 'FaceAlpha', 0.1); hold on
fill([durTotal, fliplr(durTotal)], ...
         [Total_intertemporal_hbo_upper(:,i); flipud(Total_intertemporal_hbo_lower(:,i))], ...
         [0.7, 0.1, 0.1], 'LineStyle', 'none', 'FaceAlpha', 0.1);
plot(durTotal,Total_mindfulness_hbo(:,i),'Color',[0.3, 0.4, 0.9],'linewidth',1)
plot(durTotal,Total_intertemporal_hbo(:,i),'Color',[0.7, 0.1, 0.1],'linewidth',1)
title(['Channel ',num2str(i)],'FontWeight','bold');
xlabel('Time (s)');
ylabel('HbO amplitude (uM)');
xlim([0 size(Total_mindfulness_hbo,1)/samplingrate]);
ylim([-100 100]);
xticks([0 100 200 300 400]);
yticks([-100 0 100]);
line([0 size(Total_mindfulness_hbo,1)/samplingrate], [0 0], 'Color', [0 0 0], 'LineStyle', '--');
line([1526/samplingrate 1526/samplingrate], ylim, 'Color', [0.5 0.5 0.5], 'LineStyle', '-');
line([1832/samplingrate 1832/samplingrate], ylim, 'Color', [0.5 0.5 0.5], 'LineStyle', '-');
line([2138/samplingrate 2138/samplingrate], ylim, 'Color', [0.5 0.5 0.5], 'LineStyle', '-');
set(gca,'fontsize',7,'FontName','Times New Roman', 'XColor', 'k', 'YColor', 'k');
...
grid off
box off
print(ch_plot,['ch',num2str(i)],'-dpdf','-r600')
close all
end
%% Frontopolar area
for i = [11,8]
